...
bool motif::readFimoFile(const std::string filename, const std::string path,  std::unordered_map<std::string, std::vector<double> >& ICvalues, std::unordered_map<std::string, double>& topNthreshold, bool* missingValues) {
    ...
    if (success){
        ...
        if (ICvalues.find(motifName) == ICvalues.end()) {
            ...
            *missingValues = true;
            ...
            if (IC.size() != motifLength) {
                ...
                std::cout << "Error : Motif " << motifName << "  IC values do not correspopnd to length of motif (" << motifLength << ")!" << std::endl;
        ...
        //This creares an empty vector if key (gene id) doesn't exist. Returns a poibnter to the key / new or existing vector pair
        std::pair <std::unordered_map<std::string, std::vector<motifInstance> >::iterator, bool> iter = instances.emplace(gene, std::vector<motifInstance>());
  ...
  //      motifPositions.push_back(motifInstance(first, last, strand, p1, p2, seq));
       ...
       //reads rest of file
        ...
        std::cout << "Motif " << motifName << " has " << totalHits << " occurrences in " << instances.size() << "  genes" << std::endl;
        
    }
    
    
    return success; //will be true as long as this fimo file read ok but if missingValues set then program will stop after all fimo files read
...
void motif::getListofGenes(std::vector<std::string>& genes) {
    ...
    for (std::unordered_map<std::string, std::vector<motifInstance> >::iterator it = instances.begin(); it != instances.end(); it++)
...
long motif::getNumInstances(std::string geneID)  {
 
    return instances[geneID].size();
    
}

motifInstance& motif::getInstance(std::string gene, long idx) {
    
    return instances[gene][idx];
...
double motif::getForwardICScore(int overlapLength) const {
    ...
    for (long i = 0 ; i < overlapLength; i++)
        score += IC[i];
...
double motif::getReverseICScore(int overlapLength) const {
    ...
    for (long i = (IC.size()-overlapLength) ; i < IC.size(); i++)
        score += IC[i];
