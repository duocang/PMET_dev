...
void Output::printMe(long globalBonferroniFactor, std::ofstream& outFile) {
    ...
    outFile << motif1Name << '\t' << motif2Name << '\t' << genesInClusterWithBothMotifs.size() << '\t' << numSharedGenesInUniverse << '\t' << clusterSize << '\t' << pval << '\t' << pvalBHCorrected << '\t' << pvalBonferroniCorrected << '\t' << pvalGlobalBonferroniCorrected << '\t';
...
void Output::writeHeaders(std::ofstream& of) {
    ...
    of << "Cluster\t" << "Motif 1\t" << "Motif 2\t" << "Number of genes in cluster with both motifs\t" << "Total number of genes with both motifs\t" << "Number of genes in cluster\t" << "Raw p-value\t" << "Adjusted p-value (BH)\t" << "Adjusted p-value (Bonf)\t" << "Adjusted p-value (Global Bonf)\t" "Genes" << std::endl;
