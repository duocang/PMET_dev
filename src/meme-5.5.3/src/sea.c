...
// of '?' will be used.
enum Opts {
  OPT_M, OPT_P, OPT_N, OPT_NOTRIM, OPT_ORDER, OPT_O, OPT_OC, OPT_TEXT, //OPT_OBJFUN,
...
static char *process_command_line(int argc, char* argv[], STREME_OPTIONS_T *options) {
  ...
  struct option streme_options[] = {
    ...
    {"notrim", no_argument, NULL, OPT_NOTRIM},
...
static void output_results(
  ...
  if (json_output) {
    ...
    jsonwr_bool_prop(json_output, "notrim", options->notrim);
    ...
    jsonwr_bool_prop(json_output, "noseqs", options->noseqs);
  ...
  for (imotif=0; imotif<num_motifs; imotif++) {
	...
	score_model_pssm(options, train_multiseq, model, false, false, ZOOPS, false, false, false);
...
int main(int argc, char *argv[])
{
  ...
  Multiseq *test_multiseq = read_pos_neg_seqs(&options, do_rc, True, false, options.notrim, true, back, &train_multiseq);
  ...
  for (db_i=0, i=0; db_i < arraylst_size(options.motif_sources); db_i++) {
    ...
    for (motif_i = 0; motif_i < nmotifs; motif_i++) {
      ...
      // Don't use the cache.
      if (train_multiseq) score_model_pssm(&options, train_multiseq, model, true, false, NONE, false, false, false);
      ...
      score_model_pssm(&options, test_multiseq, model, !train_multiseq, true, PASSING, true, true, false);
