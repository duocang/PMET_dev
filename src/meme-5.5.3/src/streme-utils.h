...
// Number of significant digits after rounding.
#define STREME_RND 10
...
//
// Compute the p-value of a set of sites and round it to STREME_RND places.
//
#define GET_PVALUE(log_pvalue, objfun, use_binomial, pos_count, neg_count, npos, nneg, bernoulli, dtc_sum, w, len, text, cache, cache_length) \
  { \
    ...
    RND2((log_pvalue), STREME_RND, (log_pvalue)); \
...
typedef struct options {
  ...
  BOOL parse_genomic_coord;		// parse genomic coord in primary sequence headers
  BOOL notrim;				// do not trim control sequences
...
void print_multiseq(Multiseq *multiseq); 
...
void score_model_pssm(
  ...
  BOOL pos_only                         // just score the positive sequences
);
