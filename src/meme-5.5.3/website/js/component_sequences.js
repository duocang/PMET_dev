...
BEDIH.prototype.configure = function (options) {
  ...
  // specifiy the maximum number of loci
  if (typeof options.max_loci == "number" && options.max_loci >= 1) {
    this.max_loci = options.max_loci;
  } else {
...
var SequenceInput = function(container, options) {
  ...
  this._bed_file_db_category_update();
  // add listeners
  ...
  if (typeof this.bed_file_input !== "undefined") {
    this.bed_file_db_category.addEventListener('change', function() {
      me._bed_file_db_category_update();
    }, false);
  }
  // detect BED db changes
  if (this.bed_file_db_listing != null) {
    this.bed_file_db_listing.addEventListener('change', function() {
      me._load_bed_file_db_versions(me.bed_file_db_listing.value)
    }, false);
  }
  if (this.bed_file_db_version != null) {
    this.bed_file_db_version.addEventListener('change', function() {
...
SequenceInput.prototype._source_update = function() {
  ...
  if (source == "text") {
    ...
    this._bed_file_load_listings();
    this.bed_file_validate();
  } else if (source == "db") {
 ...
 *
 ******************************************************************************/
SequenceInput.prototype._bed_file_db_category_update = function() {
  "use strict";
  if (this.source.value == "bedfile") {
    this._bed_file_load_listings();
  }
 ...
 *
 ******************************************************************************/
SequenceInput.prototype._bed_file_db_listings_update = function() {
  "use strict";
  this._load_versions();
};
 ...
 *
 ******************************************************************************/
SequenceInput.prototype._bed_file_db_versions_update = function() {
  "use strict";
};
 ...
 *
 ******************************************************************************/
SequenceInput.prototype._bed_file_load_listings = function() {
  "use strict";
  ...
  request.addEventListener("load", function(evt) {
    ...
    list.disabled = false;
    if (list.value) {
      me._load_bed_file_db_versions(parseInt(list.value, 10));
    }
  }, false);
  request.open("GET", url, true);
  request.send();
 ...
 *
 ******************************************************************************/
SequenceInput.prototype._load_bed_file_db_versions = function(listing_id) {
  "use strict";
  var me, request, url, i, list, optgroup, selected_version_id;
  // so we can access in the inner function
  me = this;
