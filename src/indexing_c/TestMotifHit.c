...
int main() {
    ...
    printMotifHit(stdout, &sampleHit);
    ...
    printMotifHit(stdout, &anotherHit);
    ...
    // anotherHit and sampleHit were initialized with allocating ram
    // freeMotifHit(&anotherHit);
    // freeMotifHit(&sampleHit);
    freeMotifHit(&hit);

    return 0;
}

// clang -o test TestMotifHit.c MotifHit.c
