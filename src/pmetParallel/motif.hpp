...
public:
    motifInstance(int start, int end, std::string str, double p1, double p2, std::string seq) : startPos(start), endPos(end), strand(str), pVal(p1), adjPVal(p2), sequence(seq){};
...
public:
    bool readFimoFile(const std::string filename, const std::string path, std::unordered_map<std::string, std::vector<double>> &ICvalues, std::unordered_map<std::string, double> &topNthreshold, bool *missingValues);
